...
struct Client {
    ...
    Client() : balance(0), hasCredit(false), creditAmount(0),
        remainingAmount(0), creditTerm(0), interestRate(0), monthlyPayment(0), monthsPaid(0) {
    }
};
...
private:
    ...
    void recalculateMonthlyPayment() {
        if (currentClient != nullptr && currentClient->hasCredit) {
            int remainingYears = currentClient->creditTerm - (currentClient->monthsPaid / 12);
            if (remainingYears <= 0) remainingYears = 1;
            currentClient->monthlyPayment = calculateMonthlyPayment(
                currentClient->remainingAmount,
                remainingYears,
                currentClient->interestRate
            );
        }
    }
...
public:
    ...
    void showAvailableCredits() {
        ...
        std::cout << "Maximum credit amount: 3,000,000 RUB\n" << std::endl;
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                double requiredForSix = payment * 6;

                bool available = (currentClient->balance >= requiredForSix);

                std::cout << "  - Amount " << ranges[j] << " RUB, rate " << rate * 100
                    << "%, monthly payment ~" << (int)payment << " RUB";

                if (available) {
                    std::cout << " [AVAILABLE for you]";
                }
                else {
                    std::cout << " [NOT AVAILABLE: need " << (int)(requiredForSix - currentClient->balance) << " more RUB for 6 payments]";
                }
                std::cout << std::endl;
            }
            std::cout << std::endl;
    ...
    void checkCreditStatus() {
        ...
        if (currentClient->hasCredit) {
            ...
            std::cout << "Monthly payment: " << currentClient->monthlyPayment << " RUB" << std::endl;
    ...
    bool checkCreditEligibility(double amount, int termYears) {
        ...
        if (amount > 3000000) {
            std::cout << "Error: maximum credit amount is 3,000,000 RUB!" << std::endl;
            return false;
        }
        ...
        std::cout << "Requested amount: " << amount << " RUB" << std::endl;
    ...
    bool takeCredit(double amount, int termYears) {
        ...
        currentClient->monthlyPayment = monthlyPayment;
    ...
    void showCurrentCreditState() {
        ...
        std::cout << "Initial credit amount: " << currentClient->creditAmount << " RUB" << std::endl;
        std::cout << "Remaining debt: " << currentClient->remainingAmount << " RUB" << std::endl;
        std::cout << "Current monthly payment: " << currentClient->monthlyPayment << " RUB" << std::endl;
    ...
    bool payMonthlyPayment(double amount) {
        ...
        if (amount < currentClient->monthlyPayment) {
            std::cout << "Error: payment amount (" << amount << " RUB) is less than accrued ("
                << currentClient->monthlyPayment << " RUB)!" << std::endl;
            return false;
        }
        ...
        double principal = currentClient->monthlyPayment - interest;

        if (amount > currentClient->monthlyPayment) {
            double extraPayment = amount - currentClient->monthlyPayment;
        ...
        std::cout << "Interest paid: " << interest << " RUB" << std::endl;
        std::cout << "Principal paid: " << principal << " RUB" << std::endl;
        std::cout << "Remaining debt: " << currentClient->remainingAmount << " RUB" << std::endl;
        std::cout << "Your balance: " << currentClient->balance << " RUB" << std::endl;
        ...
        if (currentClient->remainingAmount <= 0.01) {
            ...
            currentClient->monthlyPayment = 0;
            currentClient->monthsPaid = 0;
            std::cout << "Congratulations! Credit fully paid off!" << std::endl;
        }
        else {
            recalculateMonthlyPayment();
            std::cout << "New monthly payment: " << currentClient->monthlyPayment << " RUB" << std::endl;
        }
...
public:
    ...
    void addTestClients() {
        ...
        addClient("0004", "Kuznetsov Alexey Dmitrievich", 2500000, "strongpass");
    }
    ...
    void run() {
        ...
        std::cout << "=== BANK PROCESSING CENTER (CREDITS) ===" << std::endl;
            ...
            if (choice == 1) {
                ...
                if (authorizeClient(account, password)) {
                    ...
                    do {
                        ...
                        std::cout << "1 - Show available credits (considering your balance)" << std::endl;
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            std::cout << "Enter credit amount (max 3,000,000 RUB): ";
                            std::cin >> amount;
